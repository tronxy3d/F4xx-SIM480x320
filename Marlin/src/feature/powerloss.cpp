 ...
 */
#if TRONXY_UI
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=0.0*/) {
#else
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=POWER_LOSS_ZRAISE*/, const bool raised/*=false*/) {
  ...
  if (force
    ...
    TERN_(HAS_POSITION_SHIFT, info.position_shift = position_shift);
      ...
      #if HAS_MULTI_EXTRUDER || TRONXY_UI
...
void PrintJobRecovery::resume() {
  ...
  gcode.process_subcommands_now_P(PSTR("G92.9E0")); // Reset E to 0
    ...
    #if Z_HOME_DIR > 0
      gcode.process_subcommands_now_P(PSTR("G28R0 S0"));
      while(queue.has_commands_queued() || planner.has_blocks_queued())watchdog_refresh();
  ...
  #if TRONXY_UI
  feedrate_mm_s = SQRT(info.nominal_speed_sqr);
  #else
