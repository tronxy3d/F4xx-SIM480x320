  ...
  for (uint8_t cnt = 0; dir.readDir(&p, longFilename) > 0;) {
    if (is_visible_entity(p)) {
      if (cnt == index) {
        ...
        #if TRONXY_UI
        for(uint16_t *t = longFilename; *t; t++) {
          if(t[0] == '.' && (t[1] == 'g' || t[1] == 'G') &&
          (t[2] == 'c' || t[2] == 'C') &&
          (t[3] == 'o' || t[3] == 'O')) {
            t[2] = 0;
...
void CardReader::mount() {
  ...
  if(xy_device_id)driver = &card.media_driver_usbFlash;
  else driver = &card.media_driver_sdcard;
  #endif
  flag.mounted = false;
...
void CardReader::manage_media() {
  ...
  uint8_t stat;
  #if USBHOST_HS_EN
  if(xy_device_id == 1)stat = DiskIODriver_USBFlash::isInserted();
  else
  #endif
  {
    #if HAS_SD_DETECT
      if(READ(SD_DETECT_PIN) == SD_DETECT_STATE)stat = 1;
      else stat = 0;
    #else
      stat = 1;
    #endif
