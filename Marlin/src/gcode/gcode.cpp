...
#endif
#define IS_SPACE(x) (x == ' ' || x == '\t' || x == '\f' || x == '\r' || x == '\n')
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 999: M999(); break;                                    // M999: Restart after being Stopped
        ...
        case 8111: //设置进料速比
          ...
          if(parser.seen('S')) {
            ...
            if(v > 0) {
              if(v > 40.0)v = 40.0;
              fifth_e_factor = v;
              SERIAL_ECHO("M8111 S0");
              break;
            }
          }
          SERIAL_ECHO("M8111 S1");
          #endif
          break;
        case 8112:
          #if TAOLI_SERIAL
        ...
        case 8113:
          #if TAOLI_SERIAL
          SERIAL_ECHOPAIR_F("M8113-",fifth_e_factor);
          #endif
          break;
        ...
        #if WIFI_INTERFACE
        case 8299:
          if(sta.wifi_exe == WIFI_EXE_INFO_START) {
            int i;
            char astr[32];
            ...
            if(*tstr) {
              ...
              astr[i] = 0;
              lbComponents[2]->setChars(astr);
              lbComponents[3]->setWordsColor(COLOR_GREEN,scaBaseIdle.bgStart);
              lbComponents[3]->setWBuffer(23,"Connect");
            }
            else {
              lbComponents[2]->setChars("---");
              if(lbComponents[3]->words[0] == 'C' && lbComponents[3]->words[1] == 'o') {
                lbComponents[3]->setWordsColor(COLOR_RED,scaBaseIdle.bgStart);
                lbComponents[3]->setWBuffer(23,"Disconnect");
              }
            }
          }
          break;
        case 8400:
          if(sta.wifi_exe == WIFI_EXE_SCAN_START && sta.scanIndex < WIFI_Y_NUM) {
            lbComponents[sta.scanIndex]->setChars(parser.string_arg);
            sta.scanIndex++;
          }
          WIFI_EXPIRE_RERESH();
          break;
          ...
          if(sta.wifi_exe == WIFI_EXE_INFO_START) {
            ...
            for(int t = 0; t < 2; t++) {
              ...
              if(*tstr) {
                ...
                astr[i] = 0;
                lbComponents[t]->setChars(astr);
              }
              else lbComponents[t]->setChars("---");
            }
          }
          break;
        case 8402:
          sta.wifi_st = (WIFI_ST)parser.intval('W');
          sta.socket_st = (SOCKET_ST)parser.intval('S');
          WIFI_EXPIRE_RERESH();
          break;
        case 8409:
          sta.wifi_exe = WIFI_EXE_LOGIN_END;
          break;
        case 8410:
          sta.wifi_exe = WIFI_EXE_STOP_END;
          break;
        #endif

        #if ENABLED(Z_MULTI_ENDSTOPS) && ENABLED(FIX_MOUNTED_PROBE)
        case 8800:
          if(parser.seen('L')) {
            float l = parser.floatval('L');
            probe.deploy();
            if(parser.seen('F'))probe.probe_down_to_z(-l,MMM_TO_MMS(Z_PROBE_FEEDRATE_FAST));
            else probe.probe_down_to_z(-l,MMM_TO_MMS(Z_PROBE_FEEDRATE_SLOW));
            probe.stow();
          }
          break;
        #endif
        case 8885:
          do{
            uint8_t W25QxxEraseSector(uint32_t sectorAddr);
            uint32_t addr = parser.ulongval('A');
            if(W25QxxEraseSector(addr))SERIAL_IMPL.println("Erase NG.");
            else SERIAL_IMPL.println("Erase OK.");
          }while(0);
          break;
        case 8886:
          do {
            uint32_t addr = parser.ulongval('A');
            uint32_t val;
            EROM_ReadStart(addr,&val,4,100);
            while(!EROM_ReadOk()){}
            SERIAL_IMPL.printf("Read:0x%X\r\n",val);
          }while(0);
          break;
        case 8887:
          do {
            uint32_t addr = parser.ulongval('A');
            uint32_t val = parser.ulongval('V');
            uint32_t nextAddr = 1;
            EROM_SWrite(addr,nextAddr,&val,4);
          } while(0);
          break;
      #endif
      case 8888:
