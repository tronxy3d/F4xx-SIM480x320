...
G29_TYPE GcodeSuite::G29() {
  ...
  #else // !PROBE_MANUALLY
  {
    ...
    abl.measured_z = 0;
      ...
      for (PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_SIZE && !isnan(abl.measured_z); PR_OUTER_VAR++) {
        ...
        for (PR_INNER_VAR = inStart; PR_INNER_VAR != inStop
        #if TRONXY_UI
        && !globalStop
        #endif
        ; pt_index++, PR_INNER_VAR += inInc) {
          ...
          if (isnan(abl.measured_z)) {
            ...
            #if TRONXY_UI
            if(updateAutoLevel)updateAutoLevel(abl.meshCount.x, abl.meshCount.y, (float*)z_values);
            #else
  ...
  #if TRONXY_UI
  if(updateAutoLevel) {
    if(abl.meshCount.y == abl.grid_points.y)updateAutoLevel(0, -1, (float*)z_values);//正常完成
    else updateAutoLevel(0, -2, (float*)z_values);//异常
  }
  #endif
